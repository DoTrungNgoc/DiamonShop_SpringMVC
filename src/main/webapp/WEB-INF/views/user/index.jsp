						...
						<c:if test="${highlight_products.size() > 0}">
							<div class="item active">
								...
								<c:forEach var="item" items="${highlight_products}" varStatus="loop">
									...
									<c:if test="${(loop.index+1)%4==0 || (loop.index+1)==highlight_products.size()}">
										</ul>
										</div>
						...
						<c:forEach var="item" items="${new_products}" varStatus="loop">
							<li class="span4">
								<div class="thumbnail">
									<a class="zoomTool" href="product_details.html"
										title="add to cart"><span class="icon-search"></span>
										QUICK VIEW</a> <a href="chitietsanpham/${item.id_product}"><img
							...
							<c:if
								test="${(loop.index +1)%3==0 || (loop.index +1) == new_products.size()}">
					</ul>
